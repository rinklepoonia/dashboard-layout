    ...
    <div>
      ...
      <div className='flex flex-col gap-4.5'>
        {DASHBOARD_ICON_DATA_LIST.map((obj, i) => (
          <div key={i} className=' '>
                <div className='size-11.5 bg-blue-gradient rounded-sm flex justify-center items-center '>
              <Icons className='size-[29px]' icon={obj.icon} />
            </div>
                <p className='font-medium text-light-green text-custom-xsm mt-1.5'>{obj.label}</p>
          </div>
        ))}
      </div>
    </div>
